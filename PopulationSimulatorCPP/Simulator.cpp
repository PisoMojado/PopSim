...
static std::vector<std::tuple<Person,Person>>* PartitionIntoPairs(std::vector<Person>& population)
{
	...
	while(population.size() > 1)
	{
		...
		int partner2Index = matchingGenerator(re);
		while(partner2Index == partner1Index)
		{
			partner2Index = matchingGenerator(re);
		}
		pairs->push_back(std::tuple<Person,Person>(population[partner1Index], population[partner2Index]));		
	}
	return pairs;
}
...
static void CreateInitialPopulation(int ancestorCount, std::vector<Person>& population)
{
    ...
    std::vector<Person> ancestors;// = std::vector<Person>();
